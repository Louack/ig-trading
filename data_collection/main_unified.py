#!/usr/bin/env python3
"""
Main script for the unified data collection service
"""
...
def main():
    """Main function to demonstrate unified data collection"""
    ...
    try:
        ...
        if success:
            ...
            if stored_data is not None:
                ...
                logger.info(f"Date range: {stored_data['timestamp'].min()} to {stored_data['timestamp'].max()}")
                logger.info(f"Price range: {stored_data['closePrice'].min():.1f} to {stored_data['closePrice'].max():.1f}")
                logger.info(f"Data source: {stored_data['source'].iloc[0] if 'source' in stored_data.columns else 'unknown'}")
