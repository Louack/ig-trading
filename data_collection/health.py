...
class DataSourceHealth:
    ...
    def is_healthy(
        self, max_consecutive_failures: int = 3, max_age_hours: int = 1
    ) -> bool:
        """
...
class HealthMonitor:
    ...
    def get_all_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            name: monitor.get_health_status() for name, monitor in self.monitors.items()
        }
    ...
    def get_unhealthy_sources(self) -> list:
        ...
        """
        return [
            name for name, monitor in self.monitors.items() if not monitor.is_healthy()
        ]
