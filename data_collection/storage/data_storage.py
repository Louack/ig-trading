...
class DataStorage:
    ...
    def store_market_data(self, market_data: MarketData) -> bool:
        ...
        try:
            ...
            if filepath.exists():
                ...
                logger.info(f"Appending {len(new_df)} new points to existing {len(existing_df)} points")
            else:
                combined_df = new_df
                logger.info(f"Creating new file with {len(new_df)} data points")
            
            # Add data integrity checksum
            combined_df['checksum'] = combined_df.apply(self._calculate_row_checksum, axis=1)
            ...
            logger.info(f"Stored {len(new_df)} data points for {market_data.symbol} to {filepath}")
            return True
    ...
    def store_multiple_market_data(self, market_data_list: List[MarketData]) -> Dict[str, bool]:
        """
    ...
    def load_latest_data(self, symbol: str, timeframe: str, source: Optional[str] = None) -> Optional[pd.DataFrame]:
        """
    ...
    def _validate_checksums(self, df: pd.DataFrame, symbol: str) -> None:
        ...
        if invalid_count > 0:
            logger.warning(f"Found {invalid_count} rows with invalid checksums in {symbol}")
    ...
    def list_available_symbols(self, timeframe: str, source: Optional[str] = None) -> List[str]:
        """
