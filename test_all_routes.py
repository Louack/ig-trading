...
from core.exceptions import (
    ...
    IGNotFoundError, IGServerError, IGAPIError
)
...
class RouteTester:
    ...
    def test_route(self, test_name: str, test_func, *args, **kwargs):
        ...
        try:
            ...
            self.test_results["passed"] += 1  # Validation errors are expected in some tests
            return None
        except Exception as e:
    ...
    def test_accounts_routes(self):
        ...
        self.test_route("GET /accounts/preferences", self.client.accounts.get_preferences)
        
        # Test 3: Update account preferences
        update_data = {"trailingStopsEnabled": True}
        self.test_route("PUT /accounts/preferences", self.client.accounts.update_preferences, update_data)
        
        # Test 4: Get activities (recent)
        self.test_route("GET /history/activity (recent)", self.client.accounts.get_activities)
        ...
        self.test_route("GET /history/activity (filtered)", self.client.accounts.get_activities, activity_filters)
        ...
        self.test_route(
            ...
            self.client.accounts.get_activities_by_date_range,
            "2024-01-01", "2024-12-31"  # Wrong format
        )
        
        # Test 8: Get transactions
        self.test_route("GET /history/transactions", self.client.accounts.get_transactions)
        ...
        self.test_route("GET /history/transactions (filtered)", self.client.accounts.get_transactions, transaction_filters)
        ...
        self.test_route("GET /history/transactions (invalid params)", self.client.accounts.get_transactions, invalid_filters)
    ...
    def test_dealing_routes(self):
        ...
        if positions and positions.get("positions"):
            ...
            self.test_route("GET /positions/{dealId}", self.client.dealing.get_position, deal_id)
        else:
            print("⚠️  GET /positions/{dealId} - SKIPPED (no positions available)")
            self.test_results["skipped"] += 1
        
        # Test 3: Get position with invalid deal ID
        self.test_route("GET /positions/{dealId} (invalid ID)", self.client.dealing.get_position, "")
        ...
        created_position = self.test_route("POST /positions/otc", self.client.dealing.create_position_otc, position_data)
        ...
        if created_position and created_position.get("dealReference"):
            ...
            self.test_route("GET /confirms/{dealReference}", self.client.dealing.get_deal_confirmation, deal_ref)
        else:
            print("⚠️  GET /confirms/{dealReference} - SKIPPED (no deal reference available)")
            self.test_results["skipped"] += 1
        
        # Test 6: Get deal confirmation with invalid reference
        self.test_route("GET /confirms/{dealReference} (invalid ref)", self.client.dealing.get_deal_confirmation, "INVALID_REF")
        ...
        if created_position and created_position.get("dealId"):
            ...
            self.test_route("DELETE /positions/otc", self.client.dealing.close_position_otc, close_position_data)
        else:
        ...
        created_order = self.test_route("POST /workingorders/otc", self.client.dealing.create_working_order_otc, working_order_data)
        ...
        if created_order and created_order.get("dealReference"):
            ...
            self.test_route("PUT /workingorders/otc/{dealId} (invalid ID)", self.client.dealing.update_working_order_otc, "", {"level": 24050.0})
        else:
            print("⚠️  PUT /workingorders/otc/{dealId} - SKIPPED (no working order available)")
            self.test_results["skipped"] += 1
        
        # Test 11: Delete working order with invalid ID
        self.test_route("DELETE /workingorders/otc/{dealId} (invalid ID)", self.client.dealing.delete_working_order_otc, "")
    ...
    def test_markets_routes(self):
        ...
        self.test_route("GET /markets (single epic)", self.client.markets.get_markets, epics="IX.D.NASDAQ.IFE.IP")
        ...
        self.test_route("GET /markets (with filters)", self.client.markets.get_markets, epics="IX.D.NASDAQ.IFE.IP", query_params=market_filters)
        
        # Test 3: Get market details
        self.test_route("GET /markets/{epic}", self.client.markets.get_market, "IX.D.NASDAQ.IFE.IP")
        
        # Test 4: Get market details with invalid epic
        self.test_route("GET /markets/{epic} (invalid epic)", self.client.markets.get_market, "INVALID")
        ...
        self.test_route("GET /prices/{epic}", self.client.markets.get_prices, "IX.D.NASDAQ.IFE.IP", price_params)
        
        # Test 6: Get prices by points
        self.test_route("GET /prices/{epic}/{resolution}/{numPoints}", self.client.markets.get_prices_by_points, "IX.D.NASDAQ.IFE.IP", "MINUTE", 10)
        
        # Test 7: Get prices by date range
        start_date = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S')
        end_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.test_route("GET /prices/{epic}/{resolution}/{startDate}/{endDate}", self.client.markets.get_prices_by_date_range, "IX.D.NASDAQ.IFE.IP", "MINUTE", start_date, end_date)
        
        # Test 8: Get prices with invalid resolution
        self.test_route("GET /prices/{epic}/{resolution}/{numPoints} (invalid resolution)", self.client.markets.get_prices_by_points, "IX.D.NASDAQ.IFE.IP", "INVALID", 10)
    ...
    def test_watchlists_routes(self):
        ...
        watchlists = self.test_route("GET /watchlists", self.client.watchlists.get_watchlists)
        ...
        created_watchlist = self.test_route("POST /watchlists", self.client.watchlists.create_watchlist, watchlist_data)
        ...
        if created_watchlist and created_watchlist.get("watchlistId"):
            ...
            self.test_route("GET /watchlists/{watchlistId}", self.client.watchlists.get_watchlist, watchlist_id)
            
            # Test 4: Add market to watchlist
            add_market_data = {"epic": "IX.D.NASDAQ.IFE.IP"}
            self.test_route("PUT /watchlists/{watchlistId}", self.client.watchlists.add_market_to_watchlist, watchlist_id, add_market_data)
            
            # Test 5: Remove market from watchlist
            self.test_route("DELETE /watchlists/{watchlistId}/{epic}", self.client.watchlists.remove_market_from_watchlist, watchlist_id, "IX.D.NASDAQ.IFE.IP")
            
            # Test 6: Delete watchlist
            self.test_route("DELETE /watchlists/{watchlistId}", self.client.watchlists.delete_watchlist, watchlist_id)
        else:
            print("⚠️  Watchlist operations - SKIPPED (no watchlist created)")
            self.test_results["skipped"] += 3
        
        # Test 7: Get watchlist with invalid ID
        self.test_route("GET /watchlists/{watchlistId} (invalid ID)", self.client.watchlists.get_watchlist, "")
        
        # Test 8: Remove market with invalid epic
        self.test_route("DELETE /watchlists/{watchlistId}/{epic} (invalid epic)", self.client.watchlists.remove_market_from_watchlist, "123", "INVALID")
