...
class PathValidators:
    ...
    def validate_epic(epic: str) -> str:
        ...
        if not re.match(r"^[A-Za-z0-9._]{6,30}$", epic):
            raise IGValidationError(
                f"Invalid epic format: {epic}. Must be 6-30 characters, alphanumeric with dots and underscores only."
            )
        return epic
    ...
    def validate_deal_id(deal_id: str) -> str:
        ...
        if not re.match(r"^.{1,30}$", deal_id):
            raise IGValidationError(
                f"Invalid deal ID format: {deal_id}. Must be 1-30 characters."
            )
        return deal_id
    ...
    def validate_watchlist_id(watchlist_id: str) -> str:
        ...
        if not watchlist_id or len(watchlist_id.strip()) == 0:
            raise IGValidationError(
                f"Invalid watchlist ID: {watchlist_id}. Cannot be empty."
            )
    ...
    def validate_date_format(date_str: str) -> str:
        ...
        if not re.match(r"^\d{2}-\d{2}-\d{4}$", date_str):
            raise IGValidationError(
                f"Invalid date format: {date_str}. Must be in dd-mm-yyyy format."
            )
        return date_str
...
class QueryValidators:
    ...
    def validate_page_size(page_size: int) -> int:
        ...
        if not isinstance(page_size, int) or page_size < 0:
            raise IGValidationError(
                f"Invalid page_size: {page_size}. Must be a non-negative integer."
            )
        return page_size
    ...
    def validate_page_number(page_number: int) -> int:
        ...
        if not isinstance(page_number, int) or page_number < 1:
            raise IGValidationError(
                f"Invalid page_number: {page_number}. Must be a positive integer."
            )
        return page_number
